...
| 行为历史 (BehaviorHistory) | ✅ | ✅ | 已实现，默认通过 feature gate 关闭 |
...
| `/api/v1/behavior-history` | GET/POST | ✅ | ✅* | Rust 侧 `behavior-history` feature 开启时可用 |
| `/api/v1/behavior-history/{id}` | DELETE | ✅ | ✅* | 软删除 |
| `/api/v1/behavior-history/stats` | GET | ✅ | ✅* | 类型统计 |
| `/health` | GET | ✅ | ✅ | |
...
| 功能 | Python | Rust | 备注 |
|------|:------:|:----:|------|
| Prometheus 监控 | ✅ | 部分 | Rust 已接入 OTEL traces/metrics，Prometheus 直出仍待补 |
| Rate Limiting | ✅ | ❌ | 待实现 |
| 授权层 (Auth) | ✅ | ❌ | 待实现 |
| 多租户隔离 | ✅ | 部分 | Rust 有 tenant context，未完整隔离 |
| MCP 协议适配器 | ✅ stub | ✅ | `evermemos-mcp`（stdio JSON-RPC 2.0）已可用 |
...
| 优先级 | 功能 | 工作量 |
|--------|------|--------|
| P1 | Rate Limiting 中间件 | 中 |
| P2 | 授权层 (Auth) | 中 |
| P3 | 多租户完整隔离 | 大 |
| P4 | Prometheus 直出 / 采集策略定型 | 中 |
| P5 | Kafka Consumer（或明确维持 NATS 路线） | 大 |
